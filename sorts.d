...
//version = Jeca;
version(Jeca) import jeca.all;

version(Jeca) mixin(Jeca.pragmalibs);
...
void main(string[] args)
{
 ...
 Mt19937 gen;
 double[] vals;
 vals.length = DISPLAY_H;
 for(int i = 0; i < vals.length; i++) vals[i] = uniform(0.0,cast(double)DISPLAY_W / 2 - 10);

	void display(double[] vals, double tab) {
		version(Jeca)
			foreach(y, w; vals)
				al_draw_line(tab, y, tab + w, y, Colour.amber, 1);
	}
